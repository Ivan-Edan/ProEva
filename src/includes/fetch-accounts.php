...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $rowsHtml .= '<td class="text-center"><a style="color: black;" href="#" data-bs-toggle="modal" data-bs-target="#editAccountModal" data-email="' . htmlspecialchars($row['email']) . '" data-id="' . htmlspecialchars($row['id']) . '" data-firstname="' . htmlspecialchars($row['first_name']) . '" data-middlename="' . htmlspecialchars($row['middle_name']) . '" data-lastname="' . htmlspecialchars($row['last_name']) . '" data-role="' . htmlspecialchars($row['role']) . '" data-department="' . htmlspecialchars($row['department']) . '">' . htmlspecialchars($row['email']) . '</a></td>';
