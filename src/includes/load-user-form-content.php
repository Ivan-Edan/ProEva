...
try {
    ...
    switch ($formType) {
        case 'form1': // FORM 2: Physical and Financial Accomplishments
            $query = "
                    ...
                    WHERE ipr.details_id = ?
                                    ...
                                    // Apply user-specific or admin-specific filtering
                                    if ($role === 'user') {
                                        $query .= " AND ipr.user_id = ?"; // For users, check their ID
                                        $params[] = $userId;
                                        $types .= 'i';
                                    } else if ($role === 'admin') {
                                        $query .= " AND ipr.status = 'pending'"; // For admins, fetch only pending forms
                                    }
                                    break;
        ...
        case 'form2': // FORM 2: Physical and Financial Accomplishments
            ...
            // Apply user-specific or admin-specific filtering
            if ($role === 'user') {
                $query .= " AND fp.user_id = ?"; // For users, check their ID
                $params[] = $userId;
                $types .= 'i';
            } else if ($role === 'admin') {
                $query .= " AND fp.status = 'pending'"; // For admins, fetch only pending forms
            }
            break;
        

        // FORM 3: Exception Report
        case 'form3':
            $query = "
                SELECT
                    ex.status,
            ...
            // Apply user-specific or admin-specific filtering
            if ($role === 'user') {
                $query .= " AND ex.user_id = ?"; // For users, check their ID
                $params[] = $userId;
                $types .= 'i';
            } else if ($role === 'admin') {
                $query .= " AND ex.status = 'pending'"; // For admins, fetch only pending forms
            }
            break;
        

        // FORM 4: Project Results
        case 'form4':
            $query = "
                SELECT
                    pr.status,
            ...
            // Apply user-specific or admin-specific filtering
            if ($role === 'user') {
                $query .= " AND pr.user_id = ?"; // For users, check their ID
                $params[] = $userId;
                $types .= 'i';
            } else if ($role === 'admin') {
                $query .= " AND pr.status = 'pending'"; // For admins, fetch only pending forms
            }
            break;
